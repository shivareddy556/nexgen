        ...
        <div className=''>
          <Row className='Row_margin_view p-0 d-flex ' >
            {Ui_ux_banners.map((item, i) => {
              return (
                <Col key={i} md={4} sm={6} xs={12} data-aos="fade-up" className='text-center image_padding'>
                  <Link to={`/designs/ui_ux_designs/${item.id}`} className='design_link_style'>
                    <img src={require(`../Design_Banners/${item.pathimage}`)} className='w-100 image-responsive image_hover_effect' alt='webpage' style={{ width: "100%" }} />
                    <Link to={`/designs/ui_ux_designs/${item.id}`} className='design_link_style'>
